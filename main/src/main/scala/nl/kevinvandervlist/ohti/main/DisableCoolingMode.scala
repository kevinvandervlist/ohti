...
object DisableCoolingMode extends RunnableTask with LazyLogging {
  override def name: String = "disable-cooling-mode"
  ...
  override def apply(api: PortalAPI, settings: Settings)(implicit ec: ExecutionContext): Unit = {
    ...
    val set: Future[Either[String, Device]] = devs.flatMap {
      ...
      case Some(dev) => dev.setHeatChoice() match {
        case None => Future(Left("Cannot update device state. Is cooling mode disabled already?"))
    ...
    result match {
      ...
      case Right(d) => logger.info("Successfully updated device {}: cooling is now disabled", d.id)
    }
  }
}
